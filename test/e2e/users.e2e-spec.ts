...
describe('users', () => {
  ...
  describe('user CRUD operations by admin', () => {
    beforeEach(async () => {
  ...
  describe('user registration', () => {
    ...
    it('should login user', async () => {
      const loginDto: LoginInputDto = {
        loginOrEmail: mockUsers[0].login,
        password: mockUsers[0].password,
      };

      const res = await request(httpServer).post(paths.auth.login).send(loginDto).expect(HttpStatus.OK);
